# E5 content probes — drumjepa_v1_vcrec (epoch 19) vs no AO-JEPA
...
|---|---|---|---|---|---|
| drumjepa | linear | validation | train_kits | 0.117 | 0.259 |
| drumjepa | linear | validation | heldout_kits | 0.113 | 0.268 |
| drumjepa | linear | test | train_kits | 0.129 [0.119, 0.136] | 0.247 |
| drumjepa | linear | test | heldout_kits | 0.128 [0.117, 0.134] | 0.253 |
| drumjepa | mlp | validation | train_kits | 0.180 | 0.280 |
| drumjepa | mlp | validation | heldout_kits | 0.168 | 0.283 |
| drumjepa | mlp | test | train_kits | 0.197 [0.182, 0.206] | 0.277 |
| drumjepa | mlp | test | heldout_kits | 0.186 [0.173, 0.193] | 0.283 |
...
|---|---|---|---|---|---|
| kick | 0.4047 | 0.277 | 0.733 | 0.775 | 0.399 |
| snare | 0.3792 | 0.328 | 0.560 | 0.761 | 0.358 |
| rimshot | 0.1073 | 0.211 | 0.705 | 0.837 | 0.104 |
...
| hh_closed | 0.2669 | 0.424 | 0.728 | 0.783 | 0.334 |
| hh_open | 0.0592 | 0.000 | 0.154 | 0.436 | 0.057 |
| hh_pedal | 0.2661 | 0.086 | 0.332 | 0.417 | 0.217 |
| crash1 | 0.0182 | 0.000 | 0.039 | 0.152 | 0.014 |
| crash2 | 0.0078 | 0.000 | 0.000 | 0.173 | 0.003 |
| ride | 0.2289 | 0.487 | 0.635 | 0.782 | 0.159 |
...
|---|---|---|---|
| drumjepa | pooled | 0.131 | 0.137 |
| drumjepa | tokens | 0.316 | 0.312 |
...
|---|---|---|---|---|---|---|---|
| velocity (MIDI vel) | drumjepa | linear | validation | train_kits | 23.044 | 24.605 | 189984 |
| velocity (MIDI vel) | drumjepa | linear | validation | heldout_kits | 22.854 | 24.605 | 253312 |
| velocity (MIDI vel) | drumjepa | linear | test | train_kits | 22.319 | 24.560 | 170676 |
| velocity (MIDI vel) | drumjepa | linear | test | heldout_kits | 22.316 | 24.560 | 227568 |
| velocity (MIDI vel) | drumjepa | mlp | validation | train_kits | 25.723 | 24.605 | 189984 |
| velocity (MIDI vel) | drumjepa | mlp | validation | heldout_kits | 26.582 | 24.605 | 253312 |
| velocity (MIDI vel) | drumjepa | mlp | test | train_kits | 24.582 | 24.560 | 170676 |
| velocity (MIDI vel) | drumjepa | mlp | test | heldout_kits | 25.699 | 24.560 | 227568 |
...
| timing (ms) | drumjepa | linear | validation | train_kits | 58.362 | 63.262 | 189984 |
| timing (ms) | drumjepa | linear | validation | heldout_kits | 57.920 | 63.262 | 253312 |
| timing (ms) | drumjepa | linear | test | train_kits | 55.650 | 60.307 | 170676 |
| timing (ms) | drumjepa | linear | test | heldout_kits | 55.158 | 60.307 | 227568 |
| timing (ms) | drumjepa | mlp | validation | train_kits | 57.238 | 63.262 | 189984 |
| timing (ms) | drumjepa | mlp | validation | heldout_kits | 57.636 | 63.262 | 253312 |
| timing (ms) | drumjepa | mlp | test | train_kits | 54.716 | 60.307 | 170676 |
| timing (ms) | drumjepa | mlp | test | heldout_kits | 55.197 | 60.307 | 227568 |
...
| hihat (CC4/127) | drumjepa | linear | validation | train_kits | 0.185 | 0.193 | 97632 |
| hihat (CC4/127) | drumjepa | linear | validation | heldout_kits | 0.184 | 0.193 | 130176 |
| hihat (CC4/127) | drumjepa | linear | test | train_kits | 0.164 | 0.179 | 94080 |
| hihat (CC4/127) | drumjepa | linear | test | heldout_kits | 0.165 | 0.179 | 125440 |
| hihat (CC4/127) | drumjepa | mlp | validation | train_kits | 0.196 | 0.193 | 97632 |
| hihat (CC4/127) | drumjepa | mlp | validation | heldout_kits | 0.195 | 0.193 | 130176 |
| hihat (CC4/127) | drumjepa | mlp | test | train_kits | 0.174 | 0.179 | 94080 |
| hihat (CC4/127) | drumjepa | mlp | test | heldout_kits | 0.177 | 0.179 | 125440 |
...
Velocity MAE is also 0.1757 (drumjepa), 0.1365 (random), 0.1266 (raw_mel) in velocity/127 units.
...
AO-JEPA was skipped in this run, so the drum-JEPA vs AO-JEPA comparison E5 exists for is not answered here; drum-JEPA's linear onset macro-F1 is 0.129 [0.119, 0.136].
...
What is recoverable is which classes were struck, and only for the frequent, spectrally distinct ones: ride, hh_closed, snare score highest and crossstick, tom_hi, tom_mid lowest under the drum-JEPA linear probe, tracking how often the class occurs rather than anything about the model; and mean-pooling the 16 frequency tokens of a step throws content away — concatenating them instead lifts drum-JEPA from 0.137 to 0.312 on the same clips, so every pooled number above is a lower bound.
What is not recoverable is the fine structure of the hit: velocity MAE is 22.3 MIDI units against 24.6 for predicting the per-class training mean, first-onset timing inside the 250 ms step is 55.7 ms against 60.3 ms, and the hi-hat pedal position is 0.164 against 0.179 (CC4/127) — every one of those beats its baseline, but by 10-25%, so the probes read the event far more sharply than its dynamics or its position inside the step.
Held-out kits cost the encoders little — drumjepa 0.128 against 0.129, random 0.320 against 0.365 — but cost the raw log-mel most of its lead (0.277 against 0.583), so the spectrum's advantage is largely kit-specific while what the encoders carry about the action is not.
